...
const requestListener = function (request, response) {
  ...
  // Serve dynamic recorder-page.
    ...
    fs.readdir('json', function (error, fileNames) {
      if (error) {
        ...
        let splitDoc = recorderDocument.split("-!-");
        let files=fileNames.join('</option>'+'<option>')
        let newDoc = splitDoc[0] + files + splitDoc[1];
  ...
  // Handle POST-requests.
    ...
    request.on('end', function () {
      ...
      let noteCookie = cookie.serialize('notes', query.note_names);
      response.setHeader('Set-Cookie', noteCookie);
